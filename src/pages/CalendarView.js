...
const CalendarView = ({
  ...
  return (
    <div className="calendar-grid">
        ...
        <React.Fragment key={time}>
          ...
          {days.map(d => (
            <div
              ...
              className="cell"
              onMouseDown={() => {
                const next = timeSlots[timeSlots.indexOf(time) + 1]
                if (next) onCellSelect(d.name, time, next)
              }}
              onMouseUp={() => dragStart && onCellSelect(dragStart.day, dragStart.time, time)}
            />
          ))}
        </React.Fragment>
      ))}
      ...
      {bookings.map(b => {
        if (!timeSlots.includes(b.startTime)) return null
